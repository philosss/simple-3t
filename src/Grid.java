	...
	int row,col,diag1,diag2;
		...
		for(int player=0;player<2;player++){
			...
			for(int i=0;i<3;i++){
				...
				for(int j=0;j<3;j++){
					if(this.grid[i][j]!=null && ((player==0 && this.grid[i][j].getTipoBooleanX()) || (player==1 && !this.grid[i][j].getTipoBooleanX()))) col++;
					if(this.grid[j][i]!=null && ((player==0 && this.grid[j][i].getTipoBooleanX()) || (player==1 && !this.grid[j][i].getTipoBooleanX()))) row++;
					if((i==0 && j==2) || (i==2 && j==0) && this.grid[i][j]!=null && ((player==0 && this.grid[i][j].getTipoBooleanX()) || (player==1 && !this.grid[i][j].getTipoBooleanX()))) diag2++;
